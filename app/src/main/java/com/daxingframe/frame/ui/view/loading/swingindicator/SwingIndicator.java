package com.daxingframe.frame.ui.view.loading.swingindicator;
...
import com.daxingframe.frame.ui.view.loading.ParamsCreator;
...
public class SwingIndicator extends View{
    ...
    public SwingIndicator(Context context, AttributeSet attrs) {
        ...
        circleRadius = (int)a.getDimension(R.styleable.loadingview_circleRadius, paramsCreator.getDefaultCircleRadius());
        swingRadius = (int)a.getDimension(R.styleable.loadingview_swingRadius, paramsCreator.getDefaultSwingRadius());

        circleColor = (int)a.getColor(R.styleable.loadingview_circleColor, 0);
        if(circleColor == 0)
            circleColor = a.getResourceId(R.styleable.loadingview_circleColor, 0);
        if(circleColor == 0)
            circleColor = 0xFF7A97EA;

        int cycle = a.getInt(R.styleable.loadingview_cycle, 400);//周期，默认为2秒
        cycle = cycle/2;
