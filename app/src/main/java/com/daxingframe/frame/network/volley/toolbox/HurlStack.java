...
/**
 * 实现 HttpStack接口，利用Java的HttpURLConnection进行各种请求方式的请求。
 */
public class HurlStack implements HttpStack {
    ...
    private HttpURLConnection openConnection(URL url, Request<?> request) throws IOException {
        ...
        connection.setConnectTimeout(timeoutMs);//设置连接主机超时（单位：毫秒）
        connection.setReadTimeout(timeoutMs);//设置从主机读取数据超时（单位：毫秒）
        connection.setUseCaches(false);//请求是否使用缓存
